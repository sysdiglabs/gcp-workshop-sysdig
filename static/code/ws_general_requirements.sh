...
echo "export GCP_PROJECT_NUMBER=$(gcloud projects describe $TF_VAR_project_id --format='value(projectNumber)')"
echo "export GCP_REGION=$TF_VAR_region" | tee -a ~/.bash_profile
...
for i in "${!repoimages[@]}"; do
    ...
    repo_dest=${TF_VAR_region}-docker.pkg.dev/${TF_VAR_project_id}/${repositories[i]}/${repoimages[i]}
